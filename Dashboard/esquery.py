...
dfile = ["data.csv", "data.json", "date.txt"]

with open(dfile[0], 'a') as outfile1:
   with open(dfile[1], 'w') as outfile:
      outfile.write('[')
...
curtime = str(strftime("%m/%d/%Y %I:%M%p"))

with open(dfile[2],"w") as outfile2:
   outfile2.write("Date Updated: ")
   outfile2.write(curtime)

with open("table.csv","a") as outfile3:
   for file in dfile:
      outfile3.write(file+","+curtime+"\n")